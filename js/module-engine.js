...
const ModuleEngine = (() => {
  ...
  async function init(moduleId, steps) {
    ...
    // Auth guard — local-db.js provides Auth.requireAuth via localStorage
    const session = await Auth.requireAuth('/index.html');
    if (!session) return;
    _user = session.user;

    // All modules are unlocked for course buyers — no access check needed
