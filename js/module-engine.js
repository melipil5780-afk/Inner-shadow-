...
const ModuleEngine = (() => {
  ...
  async function init(moduleId, steps) {
    ...
    // Show skeleton immediately so page doesn't feel blank
    const container = document.getElementById('stepContent');
    if (container) {
      container.innerHTML = `
        <div style="padding:1.5rem 0">
          <div style="height:1.5rem;width:60%;background:var(--border-soft);border-radius:var(--radius-sm);margin-bottom:1rem;animation:pulse-orb 1.5s ease-in-out infinite"></div>
          <div style="height:.875rem;width:100%;background:var(--border-subtle);border-radius:var(--radius-sm);margin-bottom:.625rem;animation:pulse-orb 1.5s ease-in-out infinite"></div>
          <div style="height:.875rem;width:85%;background:var(--border-subtle);border-radius:var(--radius-sm);margin-bottom:.625rem;animation:pulse-orb 1.5s ease-in-out infinite"></div>
          <div style="height:.875rem;width:70%;background:var(--border-subtle);border-radius:var(--radius-sm);animation:pulse-orb 1.5s ease-in-out infinite"></div>
        </div>`;
    }
    UI.markLoaded(0); // show page with skeleton immediately
    ...
    // Fire all DB calls in parallel
    const isFree = FREE_MODULES.has(moduleId);
    const isDev  = DEVELOPER_EMAILS.has(_user.email);

    try {
      const [progressResult, answersResult, userStateResult] = await Promise.all([
        DB.getOneModuleProgress(_user.id, moduleId),
        DB.getWorksheetResponses(_user.id, moduleId),
        (!isFree && !isDev) ? DB.getUserState(_user.id) : Promise.resolve({ data: null })
      ]);

      _moduleState = progressResult.data || {};

      // Restore saved answers
      const saved = answersResult.data || [];
      saved.forEach(row => {
      ...
      console.error('[Engine] Init load failed:', err);
      _moduleState = {};
    }
